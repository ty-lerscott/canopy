	...
	try {
		...
		if (Array.isArray(socials) && socials.length) {
			for (const social of socials) {
				await dbClient
					.insert(tables.socials)
					.values(social)
					.onConflictDoUpdate({
						target: tables.socials.id,
						set: social,
					});
			}
		}
		...
		if (Array.isArray(education) && education.length) {
			for (const ed of education) {
				await dbClient.insert(tables.education).values(ed).onConflictDoUpdate({
					target: tables.education.id,
					set: ed,
				});
			}
		}
		...
		if (Object.values(user).some((val) => Boolean(val))) {
			const result = (await dbClient
				.insert(tables.users)
				.values(user)
				.onConflictDoUpdate({ target: tables.users.id, set: user })
